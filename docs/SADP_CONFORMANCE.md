...
4. **Tags present & used (§2.4)** → retired from the write path; legacy-source provenance moved to typed neutral tables (`catalog_legacy_provenance`, `catalog_legacy_assets`, `artifact_platforms`, `artifact_threats`); legacy priority is stored losslessly in the `priority` (`PRI-*`) column.
...
The canonical threat classification replacing the retired `Threat` tag. The
active alias table is `legacy_threat_alias`; its historical seed is retained in
immutable migrations 012–013 and renamed forward by migration 034. Any new
`THR-*` value or alias is added through a new migration and recorded here.

## 6. Historical identity exception

Migrations 007–015 and their generated mobile migration copy retain the former
source token because their checksums are an installed-database upgrade contract.
Migration 034 renames every active table/column and introduces
`catalog_artifact_id_aliases`; the token is not exposed by current schema APIs,
exports, identifiers, or documentation outside this documented compatibility
boundary.
