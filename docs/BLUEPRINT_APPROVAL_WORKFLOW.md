...
الأدوار هنا أدوار سير عمل مسجلة للتدقيق وليست نظام مصادقة. لإحكام ذلك أُضيف **مقعد تفويض** قابل للحقن ([`secureguide/authorization.py`](../secureguide/authorization.py)): تستشير الخدمة `Authorizer.authorize(actor, role, operation)` قبل كل عملية محكومة بدور. المُفوِّض الافتراضي `TrustingAuthorizer` يحافظ على السلوك السابق (يسجّل الدور للتدقيق دون إثبات الهوية)، وعند إضافة إدارة المستخدمين تُحقن طبقة المصادقة مُفوِّضًا يثبت الاستحقاق الفعلي — و`MappingAuthorizer` مثال بلا اعتماديات يفرض جدول `actor → أدوار ممنوحة` ويرفض بـ`AuthorizationError`.
